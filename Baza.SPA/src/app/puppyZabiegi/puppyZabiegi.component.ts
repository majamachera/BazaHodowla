...
@Component({
  selector: "app-puppyZabiegi",
  templateUrl: "./puppyZabiegi.component.html",
  styleUrls: ["./puppyZabiegi.component.css"],
})
export class PuppyZabiegiComponent implements OnInit {
  constructor(
  ...
  addModeV = false;
  addModeD = false;
  sortedData: DewormingPuppy[];
  sortedDataVac: VaccinationPuppy[];
  ...
  outToggleV() {
   ...
   delateDew(id: number) {
    alertify.confirm('"Czy na pewno chcesz usunąć odrobaczenie z zestawienia?', () => {
      this.actionService.deleteDewormingPuppy(id).subscribe(() => {
  ...
  delateVac(id: number) {
    alertify.confirm('Czy na pewno chcesz usunąć szczepienie z zestawienia?', () => {
      this.actionService.deleteVaccinationPuppy(id).subscribe(() => {
  ...
  loadDew() {
    this.dogService.getDewormingsPuppy().subscribe(
      (dew: DewormingPuppy[]) => {
        this.dew = dew;
      },
      (error) => {
  ...
  loadVac() {
    this.dogService.getVaccinationsPuppy().subscribe(
      (vac: VaccinationPuppy[]) => {
        this.vac = vac;
      },
      (error) => {
  ...
  loadDewsort(sort: Sort) {
    this.dogService.getDewormingsPuppy().subscribe((dew: DewormingPuppy[]) => {
        this.dew = dew;
  ...
  loadVacsort(sort: Sort) {
    this.dogService.getVaccinationsPuppy().subscribe((vac: VaccinationPuppy[]) => {
        this.vac = vac;
